    ...
    levels = {
        {
            ...
            "Удержите господствующую высоту до подхода подкрепления.\n"
            "Правильно распределите силы — каждый рубеж критически важен.",
            "Бюджет: 100 очков",
            "3 рубежа: ЗАПАДНЫЙ / ЦЕНТРАЛЬНЫЙ / ВОСТОЧНЫЙ"
        },
        {
            1,
            "ПЕРЕПРАВА",
            ...
            "Два укреплённых узла сопротивления прикрывают мост\n"
            "и правый фланг плацдарма.\n\n"
            "Захватите мост и удержите плацдарм.\n"
            "Рекомендуется использовать взводы для прорыва обороны.",
            "Бюджет: 150 очков",
            "2 рубежа: ЦЕНТРАЛЬНЫЙ / ПРАВЫЙ"
        },
    };
}
...
void Menu::drawBackground() {
    ...
    for (float x = 0; x < W; x += 80.f) {
        sf::RectangleShape ln({ 1.f, H });
        ln.setPosition({ x, 0 }); ln.setFillColor(sf::Color(18, 32, 18));
        win.draw(ln);
    }
    for (float y = 0; y < H; y += 80.f) {
        sf::RectangleShape ln({ W, 1.f });
        ln.setPosition({ 0, y }); ln.setFillColor(sf::Color(18, 32, 18));
        win.draw(ln);
    }
    ...
    drawTxt("ТАКТИЧЕСКАЯ КОМАНДНАЯ СИСТЕМА  |  ГЛАВНОЕ МЕНЮ",
        12, sf::Color(80, 112, 72), 260.f, 24.f);

    drawTxt("v1.0", 11, sf::Color(50, 90, 50), W - 70.f, 26.f);
}
...
void Menu::drawLeftPanel(float x, float y, float w, float h) {
    ...
    Para paras[] = {
        {
            "Получайте боевые задачи, изучайте обстановку и принимайте",
            14, sf::Color(195, 218, 185), 22.f
        },
        {
            "тактические решения. От вас зависит исход каждого сражения.",
            14, sf::Color(195, 218, 185), 38.f
        },
        {
            "Закупайте юнитов, формируйте отделения и взводы,",
            14, sf::Color(195, 218, 185), 22.f
        },
        {
            "расставляйте силы по рубежам — и идите в бой.",
            14, sf::Color(195, 218, 185), 50.f
        },
    };
    for (auto& p : paras) {
        drawTxt(p.text, p.sz, p.col, px, py);
        py += p.gap;
    }

    rect(px, py, w - 120.f, 2.f, sf::Color(35, 70, 35), sf::Color(0, 0, 0, 0));
    py += 16.f;
    ...
    Ctrl ctrls[] = {
        ...
        {"УБРАТЬ",            "Снять подразделение с рубежа (бюджет возвращается)"},
        {"ВЫПОЛНИТЬ ПРИКАЗ",  "Начать бой и получить оценку"},
        {"R",                 "Перезапустить текущую миссию"},
        {"ESC",               "Выход из игры"},
    };
    for (auto& c : ctrls) {
        rect(px, py, w - 120.f, 38.f, sf::Color(16, 28, 16),
            sf::Color(35, 70, 35));
    ...
    Star stars[] = {
        ...
        {"★☆☆", "С потерями — серьёзные ошибки в расстановке",sf::Color(255, 100, 50)},
...
void Menu::drawRightPanel(float x, float y, float w, float h) {
    ...
    drawTxt("Нажмите на миссию чтобы начать", 12,
        sf::Color(60, 100, 60), px, py);
}

void Menu::drawLevelCard(const LevelInfo& lv, float x, float y,
    ...
    float      thick = hovered ? 2.f : 1.f;

    rect(x, y, w, h, fillCol, bordCol, thick);
    ...
    numT.setPosition({ x + w - 100.f, y + h * 0.5f - 44.f });
    win.draw(numT);

    rect(x, y, 4.f, h, hovered ? sf::Color(90, 255, 90) : sf::Color(50, 120, 50),
        sf::Color(0, 0, 0, 0));
    ...
    drawTxt(lv.title, 26,
        hovered ? sf::Color(90, 255, 90) : sf::Color(195, 218, 185),
        px, py);
    py += 40.f;

    std::istringstream iss(lv.description);
    std::string line2;
    int lineCount = 0;
    while (std::getline(iss, line2) && lineCount < 4) {
        if (line2.empty()) { py += 8.f; continue; }
        drawTxt(line2, 12, sf::Color(130, 165, 120), px, py);
        py += 19.f;
        lineCount++;
    }
    ...
    rect(px, py, w - 40.f, 1.f, sf::Color(35, 70, 35), sf::Color(0, 0, 0, 0));
    py += 10.f;

    drawTxt(lv.budget, 11, sf::Color(255, 170, 0), px, py);
    drawTxt(lv.directions, 11, sf::Color(80, 112, 72), px + 180.f, py);

    if (hovered) {
        drawTxt("НАЧАТЬ  ->", 13, sf::Color(90, 255, 90),
            x + w - 140.f, y + h - 34.f);
    }
}
...
void Menu::handleMouseMove(sf::Vector2f pos) {
    hoveredLevel = -1;
    float x = W * 0.5f + 50.f;
    float py = 100.f + 28.f + 50.f;
    float cardH = 230.f, cardW = W * 0.5f - 100.f;

    for (int i = 0; i < (int)levels.size(); i++) {
        sf::FloatRect bounds({ x, py }, { cardW, cardH });
        if (bounds.contains(pos)) { hoveredLevel = i; break; }
        py += cardH + 20.f;
    }
}
...
void Menu::handleMouseClick(sf::Vector2f pos) {
    ...
    for (int i = 0; i < (int)levels.size(); i++) {
        sf::FloatRect bounds({ x, py }, { cardW, cardH });
        if (bounds.contains(pos)) { selectedLevel = i; return; }
        py += cardH + 20.f;
    }
}

void Menu::drawTxt(const std::string& s, unsigned sz,
...
void Menu::rect(float x, float y, float w, float h,
...
void Menu::line(sf::Vector2f a, sf::Vector2f b, sf::Color col, float thick) {
    ...
    s.setRotation(sf::degrees(std::atan2(d.y, d.x) * 180.f / 3.14159f));
    s.setFillColor(col); win.draw(s);
}