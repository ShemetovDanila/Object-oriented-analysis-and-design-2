...
struct Popup {
    ...
    int   dirIdx = -1;
    float x = 0.f, y = 0.f;
    std::vector<bool> expanded;   // развёрнут ли каждый элемент верхнего уровня
    // Для вложенных Formation (взводы): expanded детей
    std::vector<std::vector<bool>> childExpanded;

    void openFor(int dir, float px, float py, int count) {
        visible = true; dirIdx = dir; x = px; y = py;
        expanded.assign(count, true);
        childExpanded.resize(count);
    }
    void sync(int count) {
        while ((int)expanded.size() < count) expanded.push_back(true);
        childExpanded.resize(count);
    }
};
...
struct AddDrop {
    bool           visible = false;
    int            dirIdx = -1;
    int            mode = 0;     // 0=добавить на рубеж, 1=добавить юнит в Formation
    IMilitaryUnit* targetFormation = nullptr;
    float          x = 0.f, y = 0.f;
};
...
private:
    ...
    // Рекурсивная отрисовка дерева IMilitaryUnit в попапе
    float drawUnitTree(IMilitaryUnit* unit, float px, float cy,
    ...
    sf::Color roleColor(const std::string& role) const;
};
