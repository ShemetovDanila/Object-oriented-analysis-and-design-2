...
void Menu::drawBackground() {
    ...
    for(float x=0;x<W;x+=80.f){
        sf::RectangleShape ln({1.f,H});ln.setPosition({x,0});
        ln.setFillColor(sf::Color(18,32,18));win.draw(ln);
    }
    for(float y=0;y<H;y+=80.f){
        sf::RectangleShape ln({W,1.f});ln.setPosition({0,y});
        ln.setFillColor(sf::Color(18,32,18));win.draw(ln);
    }
...
void Menu::drawLevelCard(const LevelInfo&lv,float x,float y,float w,float h,bool hovered){
    ...
    numT.setPosition({x+w-100.f,y+h*0.5f-44.f}); win.draw(numT);
...
void Menu::handleMouseMove(sf::Vector2f pos){
    ...
    for(int i=0;i<(int)levels.size();i++){
        if(sf::FloatRect(x,py,cardW,cardH).contains(pos)){hoveredLevel=i;break;}
        py+=cardH+20.f;
    }
}
...
void Menu::handleMouseClick(sf::Vector2f pos){
    ...
    float cardH=230.f,cardW=W*0.5f-100.f;
    for(int i=0;i<(int)levels.size();i++){
        if(sf::FloatRect(x,py,cardW,cardH).contains(pos)){selectedLevel=i;return;}
        py+=cardH+20.f;
    }
}
